...
object Paginator {
    ...
    sealed class Action {
        ...
        data class LikeChanges(val likeSet: Set<String>) : Action()
    }
    ...
    private fun <T> reducer(
        ...
        when (action) {
            ...
            is Action.LikeChanges -> {
                when (state) {
                    is State.Data<*> -> {
                        val oldData = state.data as? List<CatUiViewModel>
                        val newData = mutableListOf<BaseUIViewModel>()
                        oldData?.let {
                            it.forEach {
                                newData.add(it.copy(isFavorite = action.likeSet.contains(it.id)))
                            }
                        }
                        State.Data(state.pageCount, newData)
                    }
    ...
    class Store<T> @Inject constructor(schedulers: SchedulersProvider) {
        ...
        fun proceed(action: Action) {
            ...
            if (newState != state || action is Action.LikeChanges) {
                state = newState
